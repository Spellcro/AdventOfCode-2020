...
let splitPasswords = passwords.map(val => val.split(" "));

// Remove colons from the split password arrays
splitPasswords.forEach(split => split[1] = split[1].substring(0,1));

splitPasswords.forEach((arr,index) => {
    
    let bounds = arr[0].split("-");
    let lowerLimit = Number(bounds[0]);
    let upperLimit = Number(bounds[1]);

    // console.log(`Checking for ${lowerLimit}-${upperLimit} copies of ${arr[1]} in ${arr[2]}`);

    let matchedCount = 0;
    let arr2Array = arr[2].split("");
    let loop = true;

    while(loop) {
        let letterMatchIndex = arr2Array.indexOf(arr[1]);
        if (letterMatchIndex >= 0) {
            arr2Array.splice(letterMatchIndex,1);
            matchedCount += 1;

            // console.log(`After ${matchedCount} matches, remaining password to check is ${arr2Array.join("")}`);
        } else {
            loop = false;
        }
    }
    console.log(`Found ${matchedCount} copies of ${arr[1]} in ${arr[2]}.`)
    passwordsChecked += 1;
    console.log(`${passwordsChecked} passwords checked.`)
    if (matchedCount < lowerLimit || matchedCount > upperLimit) {
        // console.log(`${passwords[index]}   is a failed password`);
        failedPasswords += 1;
    }
})
console.log(failedPasswords);